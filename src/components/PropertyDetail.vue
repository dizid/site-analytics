...
<template>
  <div class="space-y-4">
    ...
    <div v-if="isLoading" class="space-y-4">
      <div v-for="i in 3" :key="i" class="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-5">
        <div class="animate-pulse">
          <div class="h-3 bg-white/10 rounded w-24 mb-4" />
          <div class="space-y-2">
            <div v-for="j in 4" :key="j" class="h-4 bg-white/10 rounded" :class="j % 2 === 0 ? 'w-4/5' : 'w-full'" />
          </div>
        </div>
      </div>
    </div>
    ...
    <!-- Error state -->
    <div v-else-if="error" class="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-5">
    ...
    <template v-else-if="detail">
      ...
      <!-- Top Pages -->
      <div class="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-5">
        <h3 class="text-sm font-medium text-white/40 uppercase tracking-wider mb-3">Top Pages</h3>

        <div v-if="detail.pages.length === 0" class="text-sm text-white/40">No page data available</div>

        <table v-else class="w-full text-sm">
          <thead>
            <tr class="border-b border-white/10">
              <th class="text-left text-xs text-white/40 font-medium pb-2">Page Path</th>
              <th class="text-right text-xs text-white/40 font-medium pb-2">Views</th>
              <th class="text-right text-xs text-white/40 font-medium pb-2">Bounce Rate</th>
            </tr>
          </thead>
          <tbody>
            <tr
            ...
            >
              <td class="py-2 pr-4 text-white/90 truncate max-w-[200px]" :title="page.pagePath">
                {{ page.pagePath }}
              </td>
              <td class="py-2 text-right text-white/90 tabular-nums">
      ...
      <!-- Devices -->
      <div class="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-5">
        <h3 class="text-sm font-medium text-white/40 uppercase tracking-wider mb-3">Devices</h3>

        <div v-if="detail.devices.length === 0" class="text-sm text-white/40">No device data available</div>
        ...
        <template v-else>
          ...
          <div class="flex flex-wrap gap-4">
            <div v-for="device in detail.devices" :key="device.device" class="flex items-center gap-2">
              ...
              <span class="text-sm text-white/90 capitalize">{{ device.device }}</span>
              <span class="text-sm text-white/40 tabular-nums">{{ formatNumber(device.sessions) }}</span>
            </div>
          </div>
        </template>
      </div>
      ...
      <!-- Top Countries -->
      <div class="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-5">
        <h3 class="text-sm font-medium text-white/40 uppercase tracking-wider mb-3">Top Countries</h3>

        <div v-if="detail.countries.length === 0" class="text-sm text-white/40">No country data available</div>

        <table v-else class="w-full text-sm">
          <thead>
            <tr class="border-b border-white/10">
              <th class="text-left text-xs text-white/40 font-medium pb-2">Country</th>
              <th class="text-right text-xs text-white/40 font-medium pb-2">Sessions</th>
            </tr>
          </thead>
          <tbody>
            <tr
              ...
              class="border-b border-white/5"
            >
              <td class="py-2 text-white/90">{{ country.country }}</td>
              <td class="py-2 text-right text-white/90 tabular-nums">
